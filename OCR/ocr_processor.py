...
class OCRProcessor:
    ...
    def __init__(self):
        ...
        # Enhanced language mapping with available languages only
        self.language_codes = {
            'german': 'deu',
            'english': 'eng',
            'auto': 'deu+eng'
    ...
    async def apply_multiple_strategies(self, image: np.ndarray, quality_metrics: ImageQualityMetrics) -> List[Dict]:
        ...
        for strategy in strategies_to_use:
            try:
                # Apply preprocessing
                processed_image = self.preprocess_image_enhanced(image, strategy.preprocessing_type)
    ...
    def preprocess_image_enhanced(self, image: np.ndarray, preprocessing_type: str) -> np.ndarray:
    ...
    def _preprocess_default(self, image: np.ndarray) -> np.ndarray:
        """Default preprocessing (improved version of original)"""
        # Convert to grayscale
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    ...
    async def store_results(self, user_id: Optional[int], folder_id: Optional[int], 
                          ocr_result: Dict, ai_analysis: Dict, file_path: str):
        """Store results in database (legacy method for compatibility)"""
        try:
            # For client uploads without user_id, use default user (admin)
            if user_id is None:
